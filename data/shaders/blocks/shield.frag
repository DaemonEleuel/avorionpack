...
void main()
{
	...
	vec3 shieldColour = vec3(1.0,1.0,1.0);

#if defined (LOW_SHIELDS)
	outFragColor.rgb = vec3(0.5, 0.8, 1.0);
	outFragColor.a = min(1.0, intensity * 2.0);
#else
	mat3 tangentToWorld = mat3(normalize(tangent), normalize(bitangent), normalize(normal));
	...
	outFragColor.rgb = distortedColor * mix(shieldColour.rgb, vec3(1), foggyness);
	outFragColor.a = 1.0;
