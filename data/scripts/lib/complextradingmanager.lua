...
DEBUGLEVEL = 5              -- is overwritten by DEBUGLEVEL in complexFactory.lua 
...
function restoreTradingGoods(data)
    debugPrint(2, "restoring tradingdata")
...
function secureTradingGoods()
    debugPrint(2, "securing Tradingdata")
...
function synchTradingLists(boughtGoodsIn, soldGoodsIn, intermediateGoodsIn, calledOnServer, isRequest)
    ...
    if onServer() == true then
        if calledOnServer == false then
            ...
            debugPrint(2, "synchTradingLists ", nil, calledOnServer, isRequest)
        end
    else
...
function onSellTextEntered(textBox)
    ...
    local maxAmountPlaceable = getMaxStock(good.name) - stock;
