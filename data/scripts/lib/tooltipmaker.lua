...
local function fillDescriptions(obj, tooltip, isFighter)
	isFighter = isFighter or false
	
	local extraLines =  0
	local ignoreList = {["Burst Fire"] = true, ["Consumes Energy"] = true, ["Overheats"] = true, ["%s%% Chance of penetrating shields"] = true}
	local additional = {}
...
local function fillObjectTooltipHeader(obj, tooltip, title, validObject, typ)
	...
	if not validObject then 
		...
		line.citalic = true
		line.ctext = "this "..typ.." has been generated with no weapons"; tooltip:addLine(line)
		line.ctext = "any kind of "..typ.." must have at least one weapon"; tooltip:addLine(line)
		line.ctext = "aborting calculation phase to keep the mod active"; tooltip:addLine(line)
		line.ctext = ""; tooltip:addLine(line)
	end;
end
...
function makeTurretTooltip(turret)
	...
	local validObject = wpn and true or false
	...
	fillObjectTooltipHeader(turret, tooltip, title, validObject,"turret"); if not validObject then return tooltip end
	fillWeaponTooltipData(turret, tooltip, wpn)
	
	-- set bottom right tooltip icon /lyr
	...
	fillDescriptions(turret, tooltip)

	return tooltip
end

function makeFighterTooltip(fighter)
	local wpn = fighter:getWeapons()
	local validObject = wpn and true or false
	...
	-- fill header area and weapon data /lyr
	fillObjectTooltipHeader(fighter, tooltip, title, validObject, "fighter"); if not validObject then return tooltip end
	fillWeaponTooltipData(fighter, tooltip, wpn)
