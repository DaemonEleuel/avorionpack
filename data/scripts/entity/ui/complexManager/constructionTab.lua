...
function updatePlan()
    ...
    if complexData == nil or next(complexData) == nil then                          --inititalizing Complexdata
        ...
        if status ~= 0 then debugPrint(0, "Could not find Factory.lua on Station.")return end
    ...
    if not checkEntityInteractionPermissions(Entity(), permissions[1]) then
        constructionButton.active = false
...
function updateStationCombo()
    ...
    for _, station in pairs(entitiesInSector) do
        if station.index ~= enindex and station:hasScript("data/scripts/entity/merchants/factory.lua") then   
            if checkEntityInteractionPermissions(station, permissions[1]) then
