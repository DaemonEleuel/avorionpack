...
local scrollFrame
local split
local productionList, upgradePriceList = {},{}
...
function updateOTListdataPanel()
    ...
    --clearing old data
    for _,uiItem in pairs(productionList) do
        uiItem.visible = false
        uiItem = nil
    end
    
    for _,uiItem in pairs(upgradePriceList) do
        uiItem.visible = false
        uiItem = nil
    end
    ...
    if selectedIndex == 0 then                                          --whole Complex
        ...
        if next(consumption) then
            ...
            table.insert(productionList, pUI)
            y = y + 35
        end
        for name,amount in pairs(consumption) do
        ...
        for name,amount in pairs(productions) do
            local ingredData = name%_t..": "..amount.."/cycle"
            local pUI = scrollFrame:createLabel(vec2(x,y), ingredData, 14)
            pUI.size = vec2(lblSize,pUI.size.y)
            table.insert(productionList, pUI)
        ...
        pUI.size = vec2(lblSize,pUI.size.y)
        table.insert(upgradePriceList, pUI)
        y = y + 30
        local comboIndex = 0 
        for i = 2,10 do
            ...
            if freeEnergy + currentFabECost - fabECost > 0 then
                if i ~= size then
                    ...
                    if fabECost - currentFabECost < 0 then
                        ...
                        pUI.color = ColorRGB(0.4, 0.7, 0.0)
                    end
                else
        ...
        table.insert(upgradePriceList, changeSizeButton)
    end
end
...
function onMoveUpPressed()
    ...
    local selectedEntry = {factoryList:getEntry(factoryList.selected)}
    local aboveEntry = {factoryList:getEntry(factoryList.selected - 1)}
    local selectedFactoryData, aboveFactoryData = indexedComplexData[factoryList.selected], indexedComplexData[factoryList.selected - 1]
    factoryList:setEntry(factoryList.selected - 1, unpack(selectedEntry))
    factoryList:setEntry(factoryList.selected, unpack(aboveEntry))
    
    indexedComplexData[factoryList.selected - 1] = selectedFactoryData
    indexedComplexData[factoryList.selected] = aboveFactoryData
    
    factoryList:select(factoryList.selected - 1)
...
function onMoveDownPressed()
    ...
    local selectedEntry = {factoryList:getEntry(factoryList.selected)}
    local belowEntry = {factoryList:getEntry(factoryList.selected + 1)}
    local selectedFactoryData, belowFactoryData = indexedComplexData[factoryList.selected], indexedComplexData[factoryList.selected + 1]
    factoryList:setEntry(factoryList.selected + 1, unpack(selectedEntry))
    factoryList:setEntry(factoryList.selected, unpack(belowEntry))
    
    indexedComplexData[factoryList.selected + 1] = selectedFactoryData
    indexedComplexData[factoryList.selected] = belowFactoryData
...
function onSizeSet()
    debugPrint(2,"change Energy consumption to:", nil, factorySizeSelector.selectedIndex)
    if factoryList == nil then return end
    ...
    debugPrint(3, "pre size ", nil, factorySizeSelector.selectedIndex, size)
    data.size = size
    local energyConsumption = getEnergycostForFactory(data.factoryTyp, size)
    if energyConsumption > EnergySystem().productionRate - EnergySystem().requiredEnergy then
